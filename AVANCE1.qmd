...
```{r}
TP <- read_csv("Café (respuestas).csv")
TP
df_filtered <- TP %>% filter(str_starts(as.character(`Alumno encuestador`), "3"))
...
grafico_cafe <- ggplot(DF2, aes(x = Gasto_semanal_cafe)) +
  ...
  scale_x_continuous(breaks = seq(0, max(DF2$Gasto_semanal_cafe, na.rm = TRUE), by = 10)) +  # Breaks cada 20 unidades
  scale_y_continuous(expand = c(0, 0)) +
...
Se observa en el gráfico que la distribución del gasto semanal en café de los estudiantes de UTEC es **claramente asimétrica hacia la derecha**. Más del 50% de la muestra gasta menos o igual a la **mediana**, que es de **10 soles** por semana, lo que indica un consumo moderado entre la mayoría de los estudiantes. El **promedio** de gasto es un poco más alto, con **15.24 soles**, debido a la presencia de algunos estudiantes que reportan gastar montos significativamente mayores, como aquellos que gastan **40 soles** o más (6.38% de las unidades muestrales en total). A pesar de esta diferencia, el promedio no se aleja drásticamente de la mediana. Sin embargo, el **coeficiente de variación (102%)** indica una alta dispersión en los datos, sugiriendo que el gasto varía considerablemente entre los estudiantes.

La **moda**, que es de **20 soles**, se encuentra alejada de los otros dos descriptores. En conclusión, la mayoría de los estudiantes tiende a gastar menos de 20 soles en café por semana, con un patrón de gasto generalmente bajo, aunque existen algunos casos extremos que elevan el promedio.
...
En este gráfico que representa el \*\*tipo de ayuda económica\*\* de los alumnos de UTEC, se observa una clara diferenciación en las categorías de ayuda económica, ordenadas mayor a menor frecuencia: **"Ninguno" (159),** "**Beca" (61), "Semi-beca" (15)**. Esta distribución muestra que la mayoría de los estudiantes no reciben ningún apoyo económico de parte de la universidad, siendo el **67.66%** del total. "**Beca"** ocupa el segundo lugar, siendo el **25.96%** del total, seguido finalmente por "**Semi-beca**" (**6.29%)**. Tanto la **moda** como la **mediana** la pertenecen a "Ninguno", por lo que se puede decir en relación a la anterior esta variable es mucho mas simétrica y concentrada en la **mediana**.
...
Para un mejor análisis, haremos dos gráficas: Una con solamente los alumnos no becados y otra con los alumnos que poseen alguna beca.

```{r}
datos_filtrados <- DF2 %>%
  filter(Apoyo_economico == "Ninguno")
...
ggplot(frecuencias, aes(x = rango_gasto, y = frecuencia)) +
  ...
  labs(x = "Gasto Semanal en Café", y = "Frecuencia", title = "Frecuencia de Gasto Semanal en Café por Ayuda Económica (ninguno)") +
  theme_minimal()
```
